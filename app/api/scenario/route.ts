...
export async function POST(req: NextRequest) {
  try {
    ...
    const currentTax = filing.tax_with_reliefs ?? calculateTax(filing.gross_income - filing.epf_employee)

    const additionalDeductions = Math.min(epfContribution, 4000)
      + Math.min(lifeInsurance, 3000)
      + Math.min(educationFee, 7000)

    const newChargeableIncome = Math.max(0, filing.gross_income - filing.epf_employee - additionalDeductions)
