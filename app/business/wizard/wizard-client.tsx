...
type BusinessSmeWizardClientProps = {
  filingId: string | null
}

export default function BusinessSmeWizardClient({ filingId }: BusinessSmeWizardClientProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-6 py-12">
      <div className="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        <p className="mb-2 text-xs font-bold uppercase tracking-wider text-blue-600">SME Tax Wizard</p>
        <h1 className="mb-2 text-2xl font-bold text-slate-900">Razak Demo Wizard (SME)</h1>
        ...
        <div className="rounded-xl border border-slate-100 bg-slate-50/70 p-6">
          <h2 className="mb-2 text-lg font-semibold text-slate-900">{question.label}</h2>
          {question.hint ? <p className="mb-5 text-sm text-slate-500">{question.hint}</p> : null}
          ...
          {question.type === 'boolean' ? (
            <div className="flex gap-3">
              <button
                onClick={() => goNext(true)}
                disabled={loading}
                className="flex-1 rounded-xl border-2 border-slate-200 py-3 font-semibold transition hover:border-blue-500 hover:bg-blue-50 disabled:opacity-60"
              >
                Yes
              </button>
              <button
                ...
                disabled={loading}
                className="flex-1 rounded-xl border-2 border-slate-200 py-3 font-semibold transition hover:border-slate-400 disabled:opacity-60"
              >
                No
              </button>
            </div>
